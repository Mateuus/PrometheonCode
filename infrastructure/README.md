...
## 6.6. Ditado por voz (opcional)

O botão de microfone do chat: a pessoa fala e o texto vai aparecendo no campo,
revisando-se conforme a frase avança.

Ele depende de um serviço separado — a API do
[Applio](https://github.com/Mateuus/Applio) — que carrega um modelo de fala na
memória. Por isso está atrás de um profile e desligado por padrão: sem ele o Hub
sobe igual, e o microfone simplesmente não aparece na tela.

### Subir

No `.env` de `infrastructure/compose`:

```bash
TRANSCRIPTION_ENABLED=true
TRANSCRIPTION_URL=http://applio:8000
# Gere uma chave própria; ela é o que separa o serviço de quem alcança a rede.
TRANSCRIPTION_API_KEY=$(openssl rand -hex 32)
...
docker compose -f docker-compose.prod.yml --profile voice up -d applio
docker compose -f docker-compose.prod.yml up -d api web
```

A primeira subida demora: o contêiner carrega os pesos do modelo antes de
responder saudável. O `start_period` do healthcheck é de três minutos por causa
disso — enquanto ele não termina, `GET /v1/transcription/status` responde
`ready: false` e a interface não oferece o microfone. É o comportamento certo:
um botão que aparece antes da hora morre no primeiro clique.

### Onde ele roda

O serviço **não publica porta**. Quem fala com ele é a Hub API, pela rede
interna do Compose — o navegador nunca o alcança, e a chave dele não sai do
servidor. Publicar essa porta poria uma API que executa modelos de voz e grava
arquivos em disco ao alcance de quem chegasse ao host.

Com o Applio já rodando noutra máquina, não suba este contêiner: aponte
`TRANSCRIPTION_URL` para lá e trate o acesso com regra de firewall, do mesmo
jeito que a seção 2.1 trata o reverse proxy remoto.

### CPU ou GPU

Funciona em CPU — foi medido assim, com o modelo `small` acompanhando fala
contínua e o texto ficando cerca de dois segundos atrás da voz. Com
`large-v3-turbo` e mais de uma pessoa ditando ao mesmo tempo, a CPU vira o
gargalo: cada sessão ocupa um trabalhador de inferência.

Com uma placa disponível, descomente a seção `deploy` do serviço `applio` no
compose e troque `STREAMING_DEVICE` para `cuda`.

### Trocar o modelo

`STREAMING_MODEL_SIZE` precisa bater com o `--build-arg STREAMING_MODEL` usado
para construir a imagem. Divergir não quebra nada de imediato — e é justamente
o problema: o serviço baixa o outro modelo na primeira conexão, com alguém
esperando do outro lado.

```bash
docker build --build-arg STREAMING_MODEL=small -t mateuus27/applio-api:small .
```
...
| Sintoma | Onde olhar |
| --- | --- |
| O microfone não aparece | `GET /v1/transcription/status` — `enabled: false` é configuração do Hub; `ready: false` é o serviço ainda carregando ou fora do ar |
| Aparece e falha ao clicar | `docker compose logs applio` — chave divergente entre `TRANSCRIPTION_API_KEY` e o `API_KEY` do contêiner fecha a conexão com 1008 |
| Texto muito atrás da fala | Modelo grande demais para a CPU. Troque para `small` ou use GPU |
| "Acesso ao microfone negado" | Permissão do navegador. O ditado exige contexto seguro: `https`, ou `localhost` em desenvolvimento |
